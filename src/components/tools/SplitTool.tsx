...
const PagePreview = ({ file, pageIndex }: { file: File; pageIndex: number }) => {
  ...
  return (
    <div className="relative w-20 h-28 md:w-28 md:h-40 bg-slate-900/5 rounded-xl border border-white/20 overflow-hidden flex items-center justify-center shadow-inner">
      ...
      <div className="absolute bottom-1 right-1 bg-slate-900/80 backdrop-blur-md text-[8px] text-white px-1.5 py-0.5 rounded-full font-black">PG.{pageIndex}</div>
    </div>
  );
};
...
export const SplitTool: React.FC = () => {
  ...
  if (!sourceFile) return <FileUpload onFilesSelected={(files) => setSourceFile(files[0])} />;

  return (
    <div className="w-full space-y-8 animate-in fade-in duration-500 pb-12">
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
        <div>
          <h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight">Split Studio</h2>
          <p className="text-slate-500 font-semibold flex items-center text-sm md:text-base">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-4 space-y-6">
           <div className="glass-card p-8 rounded-[2rem] space-y-6 border-white/50 shadow-xl">
              <div className="flex flex-col items-center text-center space-y-4">
                 <div className="p-8 bg-primary/10 rounded-3xl shadow-inner">
                    <FileText className="w-16 h-16 text-secondary" />
                 </div>
                 <div className="space-y-1">
                    <p className="font-black text-xl truncate max-w-[200px] text-slate-900">{sourceFile.name}</p>
                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{(sourceFile.size / 1024 / 1024).toFixed(2)} MB</p>
                 </div>
              </div>
              
              <div className="pt-4 border-t border-slate-100">
                 <div 
                   className="flex items-center space-x-3 bg-slate-50 p-4 rounded-2xl cursor-pointer hover:bg-slate-100 transition-all group" 
                 ...
                 >
                   <Checkbox 
                     ...
                     id="merge-toggle"
                     className="rounded-md border-secondary w-5 h-5"
                   />
                   <div className="flex flex-col">
                      <label htmlFor="merge-toggle" className="text-sm font-black cursor-pointer text-slate-900">Merge output</label>
                      <span className="text-[10px] text-slate-400 font-bold">Combine all segments</span>
                   </div>
                 </div>
              </div>
           </div>

           <div className="glass-card p-6 rounded-2xl bg-emerald-50 border-emerald-100">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="flex items-center justify-between px-2">
             <h3 className="font-black text-xl flex items-center text-slate-900 uppercase tracking-tight">
                <Eye className="w-6 h-6 mr-3 text-secondary" /> 
                Extraction Map
             </h3>
             <Button 
                variant="ghost" 
                className="text-secondary hover:text-white hover:bg-secondary rounded-xl h-10 px-4 font-black transition-all bg-white/40"
          ...
          <div className="space-y-4">
            {extractionRanges.map((range, idx) => (
              <div key={range.id} className="glass-card p-6 md:p-8 rounded-[2rem] relative group border-white/60 shadow-lg hover:bg-white/50 transition-all">
                {extractionRanges.length > 1 && (
                  <button 
                    onClick={() => removeExtractionRange(range.id)}
                    className="absolute -top-3 -right-3 bg-white shadow-xl text-destructive p-2 rounded-full opacity-0 group-hover:opacity-100 transition-all border border-destructive/10 z-10"
                  >
                    <X className="w-4 h-4" />
                  </button>
                )}
                ...
                <div className="flex flex-col sm:flex-row gap-8 items-center">
                  <div className="flex-1 w-full space-y-6">
                    <span className="text-[8px] font-black text-secondary uppercase tracking-[0.3em] bg-secondary/10 px-4 py-1.5 rounded-full border border-secondary/5">
                      Segment {idx + 1}
                    </span>
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="flex-1 space-y-2">
                        ...
                        <Input 
                          ...
                          className="glass rounded-2xl h-12 text-xl font-black text-center text-slate-900"
                        />
                      </div>
                      <div className="pt-6 text-slate-200 font-black text-xl">→</div>
                      <div className="flex-1 space-y-2">
                  ...
                  <div className="flex space-x-4 shrink-0 bg-slate-50 p-4 rounded-2xl border border-white/30">
                    <div className="text-center space-y-2">
                      <PagePreview file={sourceFile} pageIndex={parseInt(range.start) || 1} />
                      <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block">Entry</span>
                    </div>
                    <div className="text-center space-y-2">
                      <PagePreview file={sourceFile} pageIndex={parseInt(range.end || range.start) || 1} />
                      <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block">Exit</span>
                    </div>
                  </div>
                </div>
              </div>
